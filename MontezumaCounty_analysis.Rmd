...
```{r plots, echo=FALSE}
plot(cases~date, county, pch=16, cex=.5)
...
my.counties <- c("Colorado.Mesa", "California.Santa Clara", "Colorado.La Plata", "Oregon.Lane")
...
``` {r standardized by time.since.first & min cases, include=F}
# plot(cases.since.min~time.since.first, growing, log="y", type="n")
# for(i in unique(growing$state.county[which(!growing$state.county %in% bad.counties)])){
#   lines(cases.since.min~time.since.first, growing[which(growing$state.county==i),], col="#AAAAAA66")
# }
# for(i in unique(growing$state.county[which(growing$state.county %in% bad.counties)])){
#   lines(cases.since.min~time.since.first, growing[which(growing$state.county==i),], col=mypal[1])
# }
# for(j in 1:length(my.counties)){
#   lines(cases.since.min~time.since.first, growing[which(growing$state.county==my.counties[j]),], col=mypal[1+j], lwd=2)
# }
# points(cases.since.min~time.since.first, growing[which(growing$county=="Montezuma"),], pch=3, lwd=2, type="b")
# legend('bottomright', legend=c("all counties","early deaths","Montezuma",gsub(".*\\.","",my.counties)), lwd=c(1,1,2,rep(2, length(my.counties))), pch=c(NA,NA,3,NA,NA), col=c("#AAAAAA66", mypal[1],"black", mypal[2:(length(my.counties)+1)]))
# # points(cases~time.since.first, growing[which(growing$state.county %in% bad.counties),], pch=16, col="darkred", cex=.5)
# # points(cases~time.since.first, growing[grep("Montezuma", growing$county),], pch=15, col="red", cex=1)
...
# }
# for(i in unique(growing$state.county[which(growing$state.county %in% bad.counties)])){
#   lines(new.cases~cases, growing[which(growing$state.county==i),], col=paste0(mypal[1],"66"))
# }
# # plot counties of interest
# for(j in 1:length(my.counties)){
#   lines(new.cases~cases, growing[which(growing$state.county==my.counties[j]),], col=mypal[1+j], lwd=3)
# }
# # Montezuma County
...
```{r states, include=F}
my.states <- c("California","Utah","Colorado", "Oregon")
